...
const keyLength = 32;
const ivLength = 16;
const salt = 'fixed_salt_value_do_not_change_this';
...
function decrypt(encryptedText) {
  // Retorna vazio se não houver texto para descriptografar
  if (!encryptedText) return '';
  
  // Se não for string, retorna como está (pode ser número, boolean, etc)
  if (typeof encryptedText !== 'string') return encryptedText;
  
  // Verifica se está no formato IV:DadosCriptografados
  const parts = encryptedText.split(':');
  if (parts.length !== 2 || parts[0].length !== ivLength * 2) {
    return encryptedText; // Retorna original se não estiver criptografado
  }
  ...
  try {
    ...
    return decrypted || ''; // Garante que sempre retorne string
  } catch (err) {
...
// Função para verificar se um valor parece criptografado
function isEncrypted(value) {
  if (typeof value !== 'string') return false;
  const parts = value.split(':');
  return parts.length === 2 && parts[0].length === ivLength * 2;
}
